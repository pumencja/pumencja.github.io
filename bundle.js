...
document.addEventListener("DOMContentLoaded", function (event) {
	...
	'use strict';
	var login_doc = window.document.getElementById('login');
	var name_doc = window.document.getElementById('name');
	var email_doc = window.document.getElementById('email');
	var avatar_doc = window.document.getElementById('avatar');
	var repos_doc = window.document.getElementById('listOfRepos');
	var no_repos_doc = window.document.getElementById("no_repo");

	fetch('https://api.github.com/users/' + login).then(function (res) {
		res.json().then(function (data) {
			var git_login = data.login;
			var name = data.name;
			var email = data.email;
			var avatar = data.avatar_url;
			var repo_number = data.public_repos;
			var repos = [];
			var pages = Math.ceil(repo_number / 100);
			...
			if (data.message) {
				...
				for (var p = 1; p <= pages; p++) {
					promises.push(fetch('https://api.github.com/users/' + login + '/repos?page=' + p + '&per_page=100').then(function (response) {
						return response.json();
					}));
				}

				Promise.all(promises).then(function (parsedData) {

					for (var _p = 0; _p < parsedData.length; _p++) {
						var _data = parsedData[_p];

						for (var i = 0; i < _data.length; i++) {
							repos.push({ at: _data[i].updated_at, name: _data[i].name });
						}
					}
					// sorting the repos
					...
					if (repo_number === 0) {
						...
						for (var _i = 0; _i < repos.length; _i++) {
							...
							var textnode = document.createTextNode(repos[_i].name);
